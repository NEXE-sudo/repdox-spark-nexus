...
import { useFileUpload } from "@/hooks/use-file-upoad.ts"
...
export default function Component({ 
  ...
  // Call parent callback when files change
  useEffect(() => {
    if (onFilesChange) {
      onFilesChange(files);
    }
      ...
      >
        ...
        {files.length > 0 ? (
          <div className="flex w-full flex-col gap-3">
            ...
            <div className={`grid gap-4 ${maxFiles > 1 ? 'grid-cols-2 md:grid-cols-3' : 'grid-cols-1'}`}>
              {files.map((file) => (
                <div
                  key={file.id}
          ...
          <div className="flex flex-col items-center justify-center px-4 py-3 text-center">
            ...
            <p className="mb-1.5 text-sm font-medium">Drop your {maxFiles > 1 ? 'images' : 'image'} here</p>
            <p className="text-xs text-muted-foreground">
              SVG, PNG, JPG or GIF (max. {maxSizeMB}MB)
            </p>
            <Button variant="outline" className="mt-4" onClick={openFileDialog}>
              <UploadIcon className="-ms-1 opacity-60" aria-hidden="true" />
              Select {maxFiles > 1 ? 'images' : 'image'}
            </Button>
          </div>
        )}
      </div>
